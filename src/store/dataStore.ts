...
interface DataState {
  ...
  updateStaffForecast: (targets: Record<string, number>, seedStr?: string) => void;
  regenerateStaffPlans: (seedStr?: string) => void;
}
...
export const useDataStore = create<DataState>((set) => ({
  ...
  updateStaffForecast: (targets, seedStr = 'default') => {
    ...
    set((state) => ({
      staffForecast: state.staffForecast.map((f, idx) => {
        const target = targets[f.month];
        if (!target) return f;
        const seed = `${seedStr}-${f.month}-${idx}`;
        const deterministicFactor = 0.9 + (hashCode(seed) * 0.15);
  ...
  regenerateStaffPlans: (seedStr = 'default') => {
    const hashCode = (str: string) => {
      let hash = 0;
      for (let i = 0; i < str.length; i++) {
        const char = str.charCodeAt(i);
    ...
    set((state) => {
      ...
      const baseScore = 75 + Math.floor(hashCode(seedStr) * 20);
